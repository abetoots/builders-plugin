...
class Member_Registration
{
    ...
    public function render_registration_form($user_attributes, $content = null)
    {
        ...
        $attributes['recaptcha_site_key'] = get_option('builders_plugin_recaptcha_site_key', null);

        if (isset($_REQUEST['success'])) {
            $attributes['success'] = true;
            $attributes['gender'] = $_REQUEST['gend'];
        }
    ...
    public function do_register_member($fullname, $email, $gender, $birthdate, $isStudent, $branch)
    {
        ...
        if (!get_option('allow_portal_registration')) {
            ...
            if (is_wp_error($result)) {
                ...
                $redirect_url = home_url('registration');
                $redirect_url = add_query_arg('success', $email, $redirect_url);
                $redirect_url = add_query_arg('gend', $gender, $redirect_url);
            }
        }
    ...
    public function init()
    {
        ...
        add_action('admin_post_nopriv_builders_do_action_register_gym_member', array($this, 'handle_form_response'));
    }
}
